...
import com.magicalcoder.youyaboot.core.utils.OpenBrowserUtil;
import com.magicalcoder.youyaboot.core.utils.SystemOperateUtil;
    ...
    public void onApplicationEvent(ApplicationReadyEvent contextRefreshedEvent) {
       ...
       try {
        ...
        if(autoStartExplorer){
            String youyabootUrl = String.format("http://localhost:%s%s",port,contextPath);
            log.info("正在使用浏览器打开管理后台地址："+youyabootUrl);
            OpenBrowserUtil.openURL(explorerPath,youyabootUrl);
        }
    }
}
