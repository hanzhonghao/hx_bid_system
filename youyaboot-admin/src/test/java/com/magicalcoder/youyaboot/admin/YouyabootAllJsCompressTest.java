...
/**
    自动压缩合并youyaboot-all.js;
    为了获取更高的性能。当发布上线时，youyaboot加载单独的1个js 避免加载多个js导致加载速度慢的问题
 */
public class YouyabootAllJsCompressTest {
    public static void main(String[] args) {
        String path = YouyabootAllJsCompressTest.class.getResource("/").getPath();
        File file = new File(path);
        String youyabootAdminPath = file.getParentFile().getParentFile().getAbsolutePath();
        String crudPath = String.format("%s\\src\\main\\resources\\static\\assets\\magicalcoder\\v103\\crud\\",youyabootAdminPath).replace("\\",File.separator);
        ...
        String youyabootAllMinJs = crudPath+"youya-boot-all.min.js";
        FileHelper.fastWriteFileUTF8(youyabootAllMinJs,sb.toString());
